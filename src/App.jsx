...
const SLIDES = [
  ...
  FlujoSlide, CinesSlide, RolesSlide, TechSlide, FinalSlide,
]
...
const variants = {
  ...
  exit: (dir) => ({
    ...
    scale: 0.97,
    filter: 'blur(10px)',
    transition: { duration: 0.42, ease: [0.6, 0, 1, 1] },
  }),
}
      ...
      <AnimatePresence>
        {current < SLIDES.length - 1 && (
          <motion.button
            ...
            className="fixed right-5 top-1/2 -translate-y-1/2 z-50 w-10 h-10 glass rounded-full
                       flex items-center justify-center text-white/25 hover:text-cinema-red
        ...
        {current > 0 && (
          <motion.button
            ...
            exit={{ opacity: 0, x: -16 }}
            onClick={() => goTo(current - 1)}
            className="fixed left-5 top-1/2 -translate-y-1/2 z-50 w-10 h-10 glass rounded-full
                       flex items-center justify-center text-white/25 hover:text-cinema-red
                       transition-colors duration-300"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
